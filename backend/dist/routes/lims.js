...
async function getPatientContext(labOrderId) {
    ...
    const patient = orderData.service_order?.appointment?.patient;
    if (!patient)
        return { gender: 'All', ageYears: 30 };
...
router.post('/transition', async (req, res) => {
    try {
        ...
        if (updateErr)
            throw updateErr;
        // Audit Log Row
        await logAuditTrail(labOrderId, fromStatus, targetStatus, `Transition to ${targetStatus}`, userId, comments);
...
router.post('/results/save', async (req, res) => {
    try {
        const { labOrderId, userId, results, instrumentRunId, rackPosition, equipmentId, testNotes, clinicalComments, resultStatus, qcPassed, reagentInDate, calibrationVerified, maintenanceOk, duplicateRunRequired, controlLotNo, reagentLotNo, calibrationDate, expiryDate, testMethod, analyzerChannel } = req.body;
        ...
        for (const r of results) {
            ...
            if (rangeData && rangeData.length > 0) {
                ...
                const matchingRange = rangeData.find((rg) => {
...
router.post('/results/amend', async (req, res) => {
    try {
        ...
        await logAuditTrail(labOrderId, 'Certified', 'Certified', 'Result Amendment', userId, `Amended parameter: ${parameterId}. Reason: ${reason}`);
        res.json({ success: true });
    }
    catch (err) {
...
router.get('/orders/:orderId', async (req, res) => {
    try {
      ...
      `)
        ...
        // Fetch parameters
        const serviceId = order.service_order?.service_id;
        let parameters = [];
        if (serviceId) {
...
router.post('/orders/collect', async (req, res) => {
    try {
        const { labOrderId, labOrderIds, userId, collectorBadge, collectionRemarks, identityVerified, consentObtained, samples } = req.body;
        ...
        for (const targetOrderId of ids) {
            ...
            await logAuditTrail(targetOrderId, 'Ordered', 'Collected', 'Registered Collection', userId, `Registered collection for ${orderSamples.length} specimen tubes.`);
        }
        res.json({ success: true });
    }
    catch (err) {
...
router.post('/orders/accept', async (req, res) => {
    try {
        const { labOrderId, userId, receivedBy, labSection, rejectionReason, rejectionComments, notifyPhysician, requestResample, samples } = req.body;
