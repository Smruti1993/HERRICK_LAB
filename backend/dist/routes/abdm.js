...
let tokenExpiryTime = null; // Timestamp in milliseconds
let lastLoginError = null; // Captures last authentication failure reason
// Generate or retrieve Eka Care Access Token (cached for 30 minutes)
async function getEkaAccessToken() {
    ...
    try {
        ...
        if (EKA_CLIENT_ID && EKA_CLIENT_SECRET) {
            ...
            try {
                ...
                const phrList = (data.phrAddresses ||
                    data.phr_addresses ||
                    ...
                    (data.accounts ? data.accounts.map((a) => a.phrAddress || a.phr_address) : null) ||
                    []);
                res.json({
                    success: true,
                    txnId: newTxnId,
...
router.post('/phr-login', async (req, res) => {
    try {
...
router.post('/save-demographics', async (req, res) => {
    try {
