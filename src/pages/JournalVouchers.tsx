...
export const JournalVouchers: React.FC = () => {
  ...
  const handleDeleteVoucher = async (jv: JournalVoucher) => {
    if (!window.confirm(`Delete ${jv.voucherNo}? This cannot be undone.`)) return;
    const ok = await deleteJournalVoucher(jv.id);
    if (ok) {
      showToast('success', `${jv.voucherNo} deleted.`);
      setSelectedVoucher(null);
    }
  };

  const handleDeleteAllReconcileJVs = async () => {
    const recJVs = journalVouchers.filter(v => v.voucherNo.startsWith('JV-REC-'));
    if (recJVs.length === 0) { showToast('info', 'No auto-reconcile JVs found.'); return; }
    if (!window.confirm(`Delete all ${recJVs.length} auto-reconcile JV(s)? This cannot be undone.`)) return;
    for (const jv of recJVs) { await deleteJournalVoucher(jv.id); }
    showToast('success', `${recJVs.length} auto-reconcile JV(s) deleted.`);
    setSelectedVoucher(null);
  };

  const handleDeleteAllPayoutJVs = async () => {
    const payJVs = journalVouchers.filter(v => v.voucherNo.startsWith('JV-PAY-'));
    if (payJVs.length === 0) { showToast('info', 'No auto-settlement JVs found.'); return; }
    if (!window.confirm(`Delete all ${payJVs.length} auto-settlement payout JV(s)? This cannot be undone.`)) return;
    for (const jv of payJVs) { await deleteJournalVoucher(jv.id); }
    showToast('success', `${payJVs.length} settlement payout JV(s) deleted.`);
    setSelectedVoucher(null);
  };
    ...
    <div className="flex flex-col h-full gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-shrink-0 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          {journalVouchers.some(v => v.voucherNo.startsWith('JV-REC-')) && (
            <button
              onClick={handleDeleteAllReconcileJVs}
              className="flex items-center gap-2 px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-xs font-semibold shadow-sm transition-all"
            >
              <Trash2 className="w-3.5 h-3.5" /> Clear Reconcile JVs
            </button>
          )}
          {journalVouchers.some(v => v.voucherNo.startsWith('JV-PAY-')) && (
            <button
              onClick={handleDeleteAllPayoutJVs}
              className="flex items-center gap-2 px-4 py-2 bg-orange-50 hover:bg-orange-100 text-orange-700 border border-orange-200 rounded-lg text-xs font-semibold shadow-sm transition-all"
            >
              <Trash2 className="w-3.5 h-3.5" /> Clear Settlement JVs
            </button>
          )}
          {viewMode === 'list' ? (
            <button 
                ...
                <div className="p-4 border-t border-slate-150 bg-slate-50 flex items-center justify-between flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-600 hover:text-rose-800 border border-rose-200 rounded-lg text-[10px] font-bold transition-colors"
                    >
