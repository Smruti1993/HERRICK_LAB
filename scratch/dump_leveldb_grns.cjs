...
for (const file of files) {
    if (file.endsWith('.ldb') || file.endsWith('.log')) {
        ...
        try {
            ...
            if (idx !== -1) {
                console.log(`\nFound medicore_grns in ${file} at offset ${idx}`);
                // Extract value - local storage in LevelDB has key prefix and value suffix.
                // Let's find the JSON array starting with '['
                    ...
                    if (jsonEnd !== -1) {
                        ...
                        try {
                            ...
                            parsed.forEach((g, idx) => {
                                console.log(`[${idx}] GRN No: ${g.grnNo}, Invoice No: ${g.invoiceNo}, Net: ${g.netAmount}, VendorId: ${g.vendorId}`);
                                if (g.items) {
                                    console.log(`    Items (${g.items.length}):`);
                                    g.items.forEach(item => {
                                        console.log(`      - Item ID: ${item.itemId}, CGST: ${item.cgstAmount}, SGST: ${item.sgstAmount}, IGST: ${item.igstAmount}`);
                                    });
                                }
                            });
                        } catch (pe) {
